    ...
    <>
      ...
      <div className={cardClass}>
        <div className="flex flex-col gap-4 px-5 py-4 sm:px-6 sm:py-5 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-800">Danh sách lead</h3>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              onClick={reload}
              aria-label="Làm mới"
              className="flex h-11 w-11 items-center justify-center rounded-lg border border-gray-300 bg-white text-gray-600 shadow-theme-xs transition-colors hover:bg-gray-50"
            >
        ...
        <div className={`border-t border-gray-100 transition-opacity ${loading && data ? "opacity-60" : ""}`}>
          {data ? (
            <LeadsTable
              leads={data.leads}
            ...
            <div className="px-6 py-14 text-center text-sm text-gray-400">Đang tải danh sách lead...</div>
          )}
        </div>
        ...
        {data && data.total > 0 && (
          <div className="flex flex-col items-center justify-between gap-3 border-t border-gray-100 px-5 py-4 sm:flex-row sm:px-6">
            <p className="text-sm text-gray-500">
              ...
              <span className="font-medium text-gray-700">{data.total}</span> lead
            </p>
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex h-10 items-center gap-1 rounded-lg border border-gray-300 bg-white px-3 text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-40"
              >
              ...
              {pageWindow(data.page, data.totalPages).map((n) => (
                <button
                  ...
                  className={`flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium transition-colors ${
                    n === data.page ? "bg-brand-500 text-white" : "text-gray-700 hover:bg-brand-50 hover:text-brand-500"
                  }`}
                >
                  {n}
                </button>
