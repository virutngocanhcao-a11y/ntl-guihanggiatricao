...
export default function CtaBanner({ content }: { content: CtaBannerContent }) {
  return (
    <section className="bg-[#222222] overflow-hidden">
        ...
        <div className="relative hidden sm:block h-64 sm:h-[260px] md:h-[280px] w-full sm:w-[350px] md:w-[450px] shrink-0 self-end">
          ...
          <div className="absolute bottom-0 left-0 h-[115%] w-full flex items-end justify-center pointer-events-none">
            <div className="relative w-[120%] h-full">
              <Image src="/images/cta-truck.jpg" alt="Nhất Tín Logistics Truck" fill className="object-cover rounded-tl-3xl rounded-tr-3xl shadow-lg" sizes="(max-width: 640px) 100vw, 450px" />
            </div>
          </div>
        </div>

        <div className="flex-1 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8 sm:py-12 self-center w-full">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-white md:text-4xl text-center sm:text-left">
            {content.titleLine1}
            <br />
          ...
          <button
            ...
            className="whitespace-nowrap rounded-lg bg-[#fdd800] px-8 py-4 text-base font-bold text-[#222222] hover:bg-[#ffe340] shadow-md transition-colors cursor-pointer text-center"
          >
