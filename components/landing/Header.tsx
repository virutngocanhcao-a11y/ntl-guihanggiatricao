...
export default function Header({ global }: { global?: GlobalSettings }) {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-50 bg-transparent text-white">
      ...
      {isOpen && (
        <div className="md:hidden bg-[#222222] absolute top-full left-0 right-0 border-t border-[#fdd800]/20 flex flex-col gap-4 px-6 py-6 text-[13px] font-semibold text-white/90 uppercase tracking-wider shadow-2xl">
          <a href="#giai-phap" onClick={() => setIsOpen(false)} className="hover:text-[#fdd800] transition-colors">Giải pháp</a>
          <a href="#quy-trinh" onClick={() => setIsOpen(false)} className="hover:text-[#fdd800] transition-colors">Quy trình</a>
          <a href="#faq" onClick={() => setIsOpen(false)} className="hover:text-[#fdd800] transition-colors">FAQ</a>
          <a href="#lien-he" onClick={() => setIsOpen(false)} className="hover:text-[#fdd800] transition-colors">Liên hệ</a>
        </div>
      )}
    </header>
  );
