    ...
    <header className="absolute top-0 left-0 right-0 z-50 bg-transparent text-ink">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-5">
        ...
        <nav className="hidden gap-8 text-[13px] font-bold text-navy md:flex uppercase tracking-wider">
          <a href="#giai-phap" className="hover:underline hover:decoration-gold hover:decoration-2 hover:underline-offset-4 transition-colors">Giải pháp</a>
          <a href="#quy-trinh" className="hover:underline hover:decoration-gold hover:decoration-2 hover:underline-offset-4 transition-colors">Quy trình</a>
      ...
      {isOpen && (
        <div className="md:hidden bg-white absolute top-full left-0 right-0 border-t border-gray-100 flex flex-col gap-4 px-6 py-6 text-[13px] font-bold text-navy uppercase tracking-wider shadow-2xl">
          <a href="#giai-phap" onClick={() => setIsOpen(false)} className="transition-colors">Giải pháp</a>
          <a href="#quy-trinh" onClick={() => setIsOpen(false)} className="transition-colors">Quy trình</a>
          <a href="#faq" onClick={() => setIsOpen(false)} className="transition-colors">FAQ</a>
          <a href="#lien-he" onClick={() => setIsOpen(false)} className="transition-colors">Liên hệ</a>
        </div>
      )}
    </header>
  );
