    ...
    <header className="absolute top-0 left-0 right-0 z-50 bg-transparent text-[#1a1a1a]">
        ...
        <a href="/" className="flex items-center">
          {global?.logoImage ? (
            ...
            <span className="text-[22px] font-extrabold tracking-tight text-[#1a1a1a]">
              <span className="text-[#e5a000]">Nhất Tín</span> Logistics
            </span>
          )}
        </a>
      ...
      {isOpen && (
        <div className="md:hidden bg-white absolute top-full left-0 right-0 border-t border-gray-100 flex flex-col gap-4 px-6 py-6 text-[13px] font-bold text-[#222222] uppercase tracking-wider shadow-2xl">
          <a href="#giai-phap" onClick={() => setIsOpen(false)} className="hover:text-[#e5a000] transition-colors">Giải pháp</a>
          <a href="#quy-trinh" onClick={() => setIsOpen(false)} className="hover:text-[#e5a000] transition-colors">Quy trình</a>
          <a href="#faq" onClick={() => setIsOpen(false)} className="hover:text-[#e5a000] transition-colors">FAQ</a>
          <a href="#lien-he" onClick={() => setIsOpen(false)} className="hover:text-[#e5a000] transition-colors">Liên hệ</a>
        </div>
      )}
    </header>
  );
